...
section{padding:clamp(2.8rem,5.5vw,4.6rem) 0;position:relative}
...
/* ---- page hero ---- */
.phero{background:var(--black);color:var(--on-black);padding:clamp(2.2rem,4vw,3.2rem) 0 clamp(2rem,3.2vw,2.6rem);
...
.phero h1{color:#fff;max-width:24ch;margin:.7rem 0 .85rem;font-size:clamp(1.95rem,4.2vw,3.35rem)}
...
.crumbs{font-family:var(--fm);font-size:.74rem;color:#7C807C;margin-bottom:.9rem;letter-spacing:.03em}
...
.facts div{padding:.85rem 1.6rem .85rem 0;margin-right:1.6rem;border-right:1px solid rgba(255,255,255,.14)}
.facts div:last-child{border-right:none;margin-right:0}
.facts b{display:block;font-family:var(--fd);font-weight:800;font-size:1.15rem;color:#fff;line-height:1.1}
.facts span{display:block;font-family:var(--fm);font-size:.66rem;letter-spacing:.13em;text-transform:uppercase;color:var(--on-black2);margin-top:.28rem}
@media(max-width:640px){.facts div{padding-right:1rem;margin-right:1rem}.facts b{font-size:1rem}}
...
/* ---- split ---- */
.split{display:grid;grid-template-columns:1fr 1fr;gap:clamp(1.8rem,4.5vw,3.8rem);align-items:start}
.split>.rv>.shot{position:sticky;top:5.5rem}
@media(max-width:900px){.split>.rv>.shot{position:static}}
